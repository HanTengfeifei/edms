...
const EditableCell = ({ editable, value, onChange }) => (
  <div>
    {editable
      ? <Input style={{ margin: '-5px 0' }} value={value} onChange={e => onChange(e)} />
      : value
    }
  </div>
);
...
class OrderListDetailsAddrTable extends React.Component {
  ...
  handleChange(e, key, _pmyself,column) {
    ...
    const columns = [{
      ...
      render:(text,record)=>(
        <EditableCell
          ...
          value={text}
          onChange={e => this.handleChange(e, record.key,_pmyself,'recv_sum')}
        />
      )
    },{
      title: '配送时间',
