...
import AddOrderInputTwo from  './AddOrderInputTwo';
import AddOrderTableTwo from  './AddOrderTableTwo';
...
class AddOrderAll extends React.Component {
  ...
  componentDidMount(){
    RequireUtils.baseRequire('unilateral-order/company-info',{},function (data){
      if(data.code==1) {
        this.setState({
          company_all:data.data.info,
          buyer_id:data.data.info.id,
        });
      }
      else{
        this.setState({
  ...
  handleSubmit (e){
    ...
    form.validateFields((err, values) => {
      if (err) {
          ...
          obj.order_kind=1;
          obj.order_source=1; //自提
          obj.saler_id=this.child.state.saler_id;
          obj.buyer_id=this.state.buyer_id;
          ...
          obj.price = this.child.state.price ;
          obj.scenario = 'add_cg_order' ;
        ...
        <Row gutter={8} style={{ marginTop: '10px'}}>
          <Col span={24}>
            <Card hoverable   style={{margin:0}} title="订单添加" type="card">
              ...
              <AddOrderInputTwo  ref={(AddOrderInputTwo)=>(this.AddOrderInputTwo=AddOrderInputTwo)}  pthis={this} company_all={this.state.company_all}  numberAll={this.state.countAll} countAll={this.countAll}  contact_name={this.state.contact_name} sent={this.receiveValue} sentParents={this.callback}  onRef={this.onRef}/>
              {/*<Divider></Divider>*/}
              {/*<AddOrderTableTwo  sentParents={this.callbackS}   saler_id_productions={this.state.saler_id_productions}   ref="table" onRef2={this.onRef2} info={this.state.child}/>*/}
              <Divider></Divider>
